...
**Goal:** Add `stack/simplify-path.py` with a repo-consistent local harness and a regression test that checks the prompt-derived sample cases.
...
- Inspect: `stack/simplify-path.py`
- Inspect: `stack/simplify-path.txt`

- [x] Capture initial `git status --short` before editing tracked files.
- [x] Confirm `stack/simplify-path.py` is currently just a placeholder stub.
...
- [x] Add a subprocess-based unittest that runs `stack/simplify-path.py`.
...
**Files:**
- Modify: `stack/simplify-path.py`

- [x] Implement `Solution.simplifyPath` for the canonical Unix path rules.
- [x] Add a `__main__` harness using prompt examples from `stack/simplify-path.txt`.
...
- Manual verification: `python3 stack/simplify-path.py` prints the prompt sample inputs, expected canonical paths, actual results, and `Matches expected: True` for each sample.
