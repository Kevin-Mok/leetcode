...
class TreeNode:
    """Create a binary tree node used by local examples."""
...
def build_tree(values: list[Any] | None) -> Optional[TreeNode]:
    """Build a binary tree from level-order list data."""
    if not values:
        return None
...
def serialize_tree(root: Optional[TreeNode]) -> list[Any]:
    """Convert a binary tree back into level-order list data."""
    if root is None:
        return []
    values: list[Any] = []
...
class Solution:
    def maxAncestorDiff(self, root: Optional[TreeNode]) -> int:
        ...
        A node `b` is a direct child of `a` if `b` is either `a.left` or
        `a.right`.
        ...
        if root.left is None and root.right is None:
            return 0
        if root.left:
            left_diff = max(abs(root.val - root.left.val), self.maxAncestorDiff(root.left))
        if root.right:
            right_diff = max(abs(root.val - root.right.val), self.maxAncestorDiff(root.right))
...
def main() -> None:
    """Run the generated local problem scaffold."""
    parser = argparse.ArgumentParser(description="Run the generated binary-tree scaffold.")
